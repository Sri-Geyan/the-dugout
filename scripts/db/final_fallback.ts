...
const ALIAS_MAP: Record<string, string> = {
    // Exact overrides for any remaining edge cases found
    "Waqar Salamkheil": "Waqar Salamkheil",
    "Gus Atkinson": "AAP Atkinson",
    "Brydon Carse": "BA Carse",
    "Daniel Lawrence": "DW Lawrence",
...
async function main() {
    ...
    // Find players who have NO stats (the remaining 48)
    ...
    for (const player of missingPlayers) {
        ...
        // Broad T20/IT20 fallback for these missing players
        let res = await tryFetch(dbName, "m.match_type IN ('T20', 'IT20')", player);
        let usedBroadFallback = false;
        
        if (!res.hasStats) {
            // What if they only played List A / First Class? We could try that.
            res = await tryFetch(dbName, "m.match_type IN ('ODM', 'MDM', 'Test', 'ODI')", player);
            if (res.hasStats) usedBroadFallback = true;
        }

        if (res.hasStats) {
            resolvedCount++;
            console.log(`[RESOLVED] ${player.name} -> ${dbName} (${usedBroadFallback ? 'Other Formats' : 'T20/IT20 Fallback'})`);
            toUpdate.push({
                id: player.id,
             ...
             console.log(`[FAILED] ${player.name} -> ${dbName} (No stats found in ANY format)`);
        }
    }
