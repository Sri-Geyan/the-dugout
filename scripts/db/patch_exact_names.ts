...
async function main() {
    ...
    for (const player of missingPlayers) {
        ...
        const dbNameExact = player.name;
        
        // Then try initial + last name
        const parts = player.name.split(' ');
        const dbNameInitial = parts.length >= 2 ? parts[0][0] + " " + parts.slice(1).join(' ') : player.name;

        // Try exact name in T20/IT20/ODM/MDM/Test/ODI
        let res = await tryFetch(dbNameExact, "1=1", player);
        let usedName = dbNameExact;
        
        if (!res.hasStats && dbNameExact !== dbNameInitial) {
            res = await tryFetch(dbNameInitial, "1=1", player);
            usedName = dbNameInitial;
        }

        if (res.hasStats) {
            resolvedCount++;
            console.log("[RESOLVED] " + player.name + " -> " + usedName);
            toUpdate.push({
                id: player.id,
